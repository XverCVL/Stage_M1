...
class InstanceGraphManager : public InstanceManager {
   public:
    ...
    InstanceGraphProvider* get_graph_provider(
        ...
        bool complementary = false, uint trunc = 0)
    {
        ...
        return new InstanceGraphProviderFile(filtered_names, complementary, trunc);
    }

    InstanceGraphProvider* get_alea_graph_provider(uint nb, uint nb_vertices, double proba_edge)
    {
        return new InstanceGraphProviderAlea(nb, nb_vertices, proba_edge);
    }
};
