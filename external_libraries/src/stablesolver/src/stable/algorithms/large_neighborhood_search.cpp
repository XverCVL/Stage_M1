        ...
        const LargeNeighborhoodSearchParameters& parameters)
{
    ...
    GreedyParameters greedy_params;
    greedy_params.verbosity_level = 0;
    Solution solution = greedy_gwmin(instance, greedy_params).solution;
    ...
    for (output.iterations = 1;
        ...
        for (VertexId vertex_id_2: sets_in_to_update)
            ...
            scores_in.update_key(vertex_id_2, {- (double)vertices[vertex_id_2].score / instance.vertex(vertex_id_2).weight, (double)-vertices[vertex_id_2].last_removal - 1});
        ...
        for (VertexId vertex_id: sets_in_to_update)
            ...
            //        {- (double)vertices[vertex_id].score / instance.vertex(vertex_id).weight, vertices[vertex_id].last_removal});
                    {- (double)vertices[vertex_id].score / instance.vertex(vertex_id).weight, (double)-vertices[vertex_id].last_removal - 1});
        ...
        while (!solution.feasible() && !scores_in.empty()) {
            ...
            for (VertexId vertex_id_2: sets_in_to_update) {
                scores_in.update_key(
                        ...
                        {- (double)vertices[vertex_id_2].score / instance.vertex(vertex_id_2).weight, (double)-vertices[vertex_id_2].last_removal - 1});
            }
        }
        for (VertexId vertex_id_2: sets_out_to_update) {
