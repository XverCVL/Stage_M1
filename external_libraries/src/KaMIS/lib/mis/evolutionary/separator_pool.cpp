...
void separator_pool::init(MISConfig & config, graph_access & G) {
    ...
    // NEW: compute edge density and skip KaHIP for dense graphs

    const unsigned int MIN_SEPARATOR_GRAPH_SIZE = 200;
    double density = 0.0;
    if (G.number_of_nodes() > 1) {
        density = (2.0 * G.number_of_edges())
                  / ((double)G.number_of_nodes() * (G.number_of_nodes() - 1));
    }
    const double MAX_DENSITY = 0.10; // KaHIP is unreliable above ~10% density

    if (G.number_of_nodes() < MIN_SEPARATOR_GRAPH_SIZE || density > MAX_DENSITY) {
        separators_size     = 0;
        partitions_size     = 0;
        k_separators_size   = 0;
        k_partitions_size   = 0;
...
//    k_partitions_size = config.use_multiway_vc ? config.number_of_k_partitions : 0;
    if (config.use_multiway_vc)
        scores.resize(config.multiway_blocks*config.population_size*config.number_of_k_partitions);
    else
        scores.resize(config.multiway_blocks*config.population_size*config.number_of_k_separators);
    clear_scores(config);
}

void separator_pool::generate_separators(MISConfig & config, graph_access & G) {
    internal_separators.clear();
    if (config.number_of_separators > 1000000) { // Safety cap
             config.number_of_separators = 0;
    }
    ...
    for (unsigned int i = 0; i < separators_size; ++i) {
        ...
        config.imbalance = random_functions::nextDouble(0.03, 0.1);
        separator sep; 
        forall_nodes(G, node) {
            G.setPartitionIndex(node, 0);
        } endfor
        create_separator(config, G, sep);
        if (sep.separator_size == 0) continue;
        sep.id = internal_separators.size();
        internal_separators.push_back(sep);
    }
}
...
void separator_pool::create_separator(MISConfig & config, graph_access & G, separator & sep, int number_of_blocks) {
    ...
    // If the graph is small, significantly increase the allowed imbalance
    // to prevent KaHIP from aborting.
    ...
    sep.k = k;
    if (separator_size == 0 ||
        separator_size >= (unsigned int)(G.number_of_nodes() / 2)) {
        sep.separator_map = nullptr;
        sep.separator_size = 0;
        if(separator != NULL) {
            delete[] separator;
            separator = NULL;
        }
        delete[] part;
        return;   // caller's generate_separators must handle sep.separator_size == 0
    }
