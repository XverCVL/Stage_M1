...
void reduction_evolution<reducer>::perform_evolutionary(MISConfig & mis_config,
    ...
    do {
        ...
        if (reduction_counter >= mis_config.reduction_threshold) {
            ...
            reduction_evolution<reducer> re;
...
void reduction_evolution<reducer>::build_final_solution(MISConfig & mis_config, 
    ...
    bool valid = true;
    forall_nodes(G, node) {
        if (!independent_set[node]) continue;
        forall_out_edges(G, edge, node) {
            if (independent_set[G.getEdgeTarget(edge)]) { valid = false; break; }
        } endfor
        if (!valid) break;
    } endfor
...
void reduction_evolution<reducer>::perform_local_mis(MISConfig & mis_config, graph_access & G) {
    ...
    for (unsigned int i = 0; i < repetitions; ++i) {
        ...
        // Determine available operators based on pool contents
        std::vector<int> available_operators;

        if (pool->get_separators_size() > 0) {
            available_operators.push_back(0); // Node separator
        }
        if (pool->get_partitions_size() > 0) {
            available_operators.push_back(1); // Vertex cover
        }
        if (pool->get_k_separators_size() > 0) {
            available_operators.push_back(2); // Multiway
        }

        int combine = -1;
        if (!available_operators.empty()) {
            // Pick randomly from the actually available operators
            int index = random_functions::nextInt(0, available_operators.size() - 1);
            combine = available_operators[index];
        }

        // Reproduction
        if (combine == 0 || combine == 1) { // 0 and 1 require two individuals
        ...
        // --- Crossover ---
        // If combine is still -1 (meaning ALL pools are empty), we skip crossover.
        if (combine == 0) {
            ...
            // Fallback if no crossover operators are available (e.g., tiny graph)
            // Just copy an individual to apply mutation later
            island.get_random_individuum(first);
            out.solution_size = first.solution_size;
            out.solution = new NodeID[G.number_of_nodes()];
            for (NodeID n = 0; n < G.number_of_nodes(); ++n) {
                out.solution[n] = first.solution[n];
            }
            out_second.solution = NULL;
            out_second.solution_size = 0;
        }

//        int combine;
//        if(pool->get_k_separators_size() > 0){
//            combine = random_functions::nextInt(0, 2);
//        }else{
//            combine = random_functions::nextInt(0, 1);
//        }
...
//        }
        ...
        if (decision < mis_config.flip_coin) {
            ...
            if (combine == 0 || combine == 1) island.mutate(mis_config, G, out_second);
        }
