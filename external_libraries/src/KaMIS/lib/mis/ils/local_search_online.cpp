...
void local_search_online::direct_improvement(graph_access &G,
  ...
  while (1) {
    ...
    if (solutions_found > 0) {
      ...
      // 1. Are they the same node?
      if (improv_v == improv_w) conflict = true;

      // 2. Are they neighbors of each other?
      if (!conflict) {
          forall_out_edges(G, e, improv_v) {
              if (G.getEdgeTarget(e) == improv_w) { conflict = true; break; }
          } endfor
      }

      // 3. Is improv_v already in solution or does it have neighbors in solution?
      if (!conflict && perm.is_solution_node(improv_v)) conflict = true;
      if (!conflict) {
          forall_out_edges(G, e, improv_v) {
              NodeID target = G.getEdgeTarget(e);
              if (target != x && perm.is_solution_node(target)) { conflict = true; break; }
          } endfor
      }

      // 4. Is improv_w already in solution or does it have neighbors in solution?
      if (!conflict && perm.is_solution_node(improv_w)) conflict = true;
      if (!conflict) {
          forall_out_edges(G, e, improv_w) {
              NodeID target = G.getEdgeTarget(e);
              if (target != x && perm.is_solution_node(target)) { conflict = true; break; }
          } endfor
      }
